      ...
      try {
        ...
        if (user && user.email) {
          ...
          if (
            ...
            "emailSent" in emailResult &&
            !emailResult.emailSent
          ) {
            console.warn(
              `Email notification not sent for ${type} notification to user ${userId}`
            );
          }
        }
      } catch (emailError) {
...
export const createMessageNotification = async (
  ...
  try {
    ...
    const content = `New message from ${senderName}: "${messageContent.substring(
      0,
      50
