...
export const createPaymentIntent = async (
  ...
  try {
    ...
    const stripeConnectAccountId =
      providerConnectAccount?.stripeConnectAccountId;
    ...
    console.log(
      `Creating payment intent with amount: ${amount}, stripeAmount: ${stripeAmount}`
    );
    ...
    if (stripeConnectAccountId && providerConnectAccount?.chargesEnabled) {
      paymentIntentOptions.application_fee_amount = Math.round(
        (platformFee + taxAmount) * 100
      ); // Convert to cents
...
export const confirmPayment = async (
  ...
  try {
    ...
    console.log(
      "Backend: Retrieved payment intent from Stripe:",
      paymentIntent.id
    );
    ...
    if (!payment) {
      console.error(
        "Backend: Payment record not found for payment intent ID:",
        paymentIntentId
      );
      throw new Error(
...
export const processRefund = async (
  ...
  try {
    ...
    const platformFeeRefund = Math.round(
      originalPayment.platformFee * refundRatio
    );
...
export const processProviderPayouts = async (): Promise<void> => {
  try {
    ...
    for (const payout of pendingPayouts) {
      ...
      if (
        !providerConnectAccount ||
        !providerConnectAccount.stripeConnectAccountId
      ) {
        console.log(
          `Provider ${providerId} does not have a Stripe Connect account`
        );
        continue;
      }
...
export const getUserPaymentHistory = async (
  ...
  try {
    return await Payment.find({
      userId,
      $or: [{ type: PaymentType.BOOKING }, { type: PaymentType.REFUND }],
    })
      .sort({ createdAt: -1 })
...
export const releaseProviderFunds = async (
  ...
  try {
    ...
    if (!payment) {
      console.log(
        `No payment found for booking ${bookingId} or payment already released`
      );
      return null;
    }
    ...
    console.log(
      `Released funds for booking ${bookingId} to provider ${payment.providerId}`
    );
    ...
    if (
      providerConnectAccount &&
      providerConnectAccount.payoutSchedule === "manual"
    ) {
      console.log(
        `Provider ${payment.providerId} has manual payout schedule, funds available for payout`
      );
      ...
      console.log(
        `Provider ${payment.providerId} has ${providerConnectAccount.payoutSchedule} payout schedule, funds will be transferred on next scheduled payout`
      );
    }
